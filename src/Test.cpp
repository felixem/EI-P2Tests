...
This file is part of https://github.com/felixem/EI-P2Tests.

EI-P2Tests is free software: you can redistribute it and/or modify
...
const char* DIRECTORIO_PRUEBAS ="entradas";
...
void runSuite(){
	...
	//Importar la suite de indexador;
...
bool compararSalidas(const string &dirEntrada,const string &dirSalida)
{
	...
	indexarDirectorioPorExtension(dirEntrada,EXTENSION_INDEX,ficResultados);
	indexarDirectorioPorExtension(dirSalida,EXTENSION_INDEX,ficSalidaEsperada);
...
bool testDirectorio(IndexadorHash &ind, const string& directorioPruebas,
...
bool reportarNoVacios(const string &listado, const string &nomSalida)
{
	...
	if(ficEntrada.is_open())
	{
		...
		if(ficSalida.is_open())
		{
			...
			ficSalida<<"Listado de ficheros de diferencia que reportan errores de indexación:"<<"\n";
...
bool volcarIndexacion(const IndexadorHash &ind, const string &dirEntrada, long double &tiempo)
{
	...
	bool correcto = indexarDirectorioPorExtension(dirEntrada,EXTENSION_TXT,ficListaDocs);
